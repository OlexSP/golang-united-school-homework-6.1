...
func (b *box) AddShape(shape Shape) error {
	if len(b.shapes) == b.shapesCapacity {
		return fmt.Errorf("input error: %w", errorOutOfCapacity)
	}
...
func (b *box) GetByIndex(i int) (Shape, error) {
	if len(b.shapes) <= i {
		return nil, fmt.Errorf("input error: %w", errorOutOfRange)
	}
...
func (b *box) RemoveAllCircles() error {
	...
	if len(counterSlice) == 0 {
		return fmt.Errorf("input error: %w", errorNotExist)
	}
	for _, v := range counterSlice {
		_, err := b.ExtractByIndex(v)
		if err != nil {
			return err
		}
	}
